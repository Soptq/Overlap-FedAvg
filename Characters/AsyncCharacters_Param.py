...
from Utils.Helper import remote_method, get_accuracy, remote_method_async, \
    DataAggregation, load_model, ExtraWorkLoader, adjust_learning_rate
...
class ParameterServer(nn.Module):
    ...
    def model_update(self):
        ...
        static_state_dict_update = copy.deepcopy(self.model.state_dict())
        for name, param in static_state_dict_update:
            ...
            for index in range(0, self.world_size - 1):
                if "running_var" in name:
                    aggregation_weights +=\
                        copy.deepcopy(workers_weight[index]
            ...
            if "running_mean" in name:
                static_state_dict_update[name] = aggregation_weights
            elif "running_var" in name:
                static_state_dict_update[name] = static_state_dict_update[name.replace("running_var", "running_mean")]
            else:
                static_state_dict_update[name] = copy.deepcopy(static_state_dict_update[name]
                                                               - self.mom_buffer[name]).to(self.device)
        self.model.load_state_dict(static_state_dict_update)
...
class TrainerNet(nn.Module):
    ...
    def train_locally(self, epoch, cluster_is_ready):
        ...
        logger.info(f"Rank {self.rank} communication time: {communication_end - communication_start}")
        return self.rank, train_end - train_start, communication_end - communication_start
    ...
    def fetch_state(self):
        ...
        for name, param in static_state_dict.items():
            if name not in self.prefetch_model:
                continue
            static_state_dict[name] = copy.deepcopy(self.prefetch_model[name]).to(self.device)
        self.model.load_state_dict(static_state_dict)
